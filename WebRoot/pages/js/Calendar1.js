...
var bMoveable=true;
var strFrame;		
document.writeln('<iframe id=endDateLayer frameborder=0 style="position: absolute;SCROLLING=NO; width: 162; height: 213; z-index: 9998; display: none"></iframe>');
strFrame='<style>';
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 width=142 height=150 bordercolor=#63A3E9 bgcolor=#63A3E9 >';
...
strFrame+='	   <tr><td width=142 height=20>';
...
strFrame+='<td width=20 height=20 id=meizzDay'+n+' style="font-size:12px" onclick="parent.meizzDayClick(this.innerText,0);parent.showDate(event);"></td>';n++;}
strFrame+='</tr>';}
strFrame+='      <tr align=center >';
...
function setday(tt,obj) //主调函数
{
	...
	var position_value = tt.position_;
	var ttyp  = tt.type;          //TT控件的类型
	...
	//如果分辨率为1024 可通过设置日期控件的显示高度，显示控件位置 jws
	if(screen.width <= '1024' && position_value != undefined){
		dads.top  = (ttyp=="image")? ttop+thei : ttop+thei+6-position_value;
	}else{
		dads.top  = (ttyp=="image")? ttop+thei : ttop+thei+6;
	}
...

{
	...
	meizzTheYear = outObject.value.parseDate().getFullYear();
	meizzTheMonth = outObject.value.parseDate().getMonth()+1;
	meizzTheDate = outObject.value.parseDate().getDate();
